...
- **CSS custom properties in `:root`** ([public/css/style.css](public/css/style.css)) define the entire design system: color tokens (`--blue`, `--harbour`, `--ink`, `--steel`, `--mist`, `--orange`, etc. — navy + orange taken from the EduPedia logo), font stacks (`--display` = Poppins, `--body`/`--util` = Nunito), spacing (`--gutter`, `--maxw`, `--rail`), and the shared easing curve `--ease`. Change tokens here rather than hardcoding colors/fonts in section rules.
